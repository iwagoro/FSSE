...
class Encoder(nn.Module):
    def __init__(self, in_channels, n_feat):
        ...
        self.f0 = nn.Sequential(
            CConv2d(in_channels, n_feat, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(inplace=True),
            CConv2d(n_feat, n_feat, kernel_size=2, stride=2, padding=0),
            nn.LeakyReLU(inplace=True),
        )

        self.f1 = nn.Sequential(
            CConv2d(n_feat, n_feat, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(inplace=True),
            CConv2d(n_feat, n_feat, kernel_size=2, stride=2, padding=0),
            nn.LeakyReLU(inplace=True),
        )
...
class Decoder(nn.Module):
    def __init__(self, dim, out_channels):
        ...
        self.d0 = nn.Sequential(
            ...
            nn.LeakyReLU(inplace=True),
            CConv2d(dim, dim, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(inplace=True),
        )
        ...
        self.out = CConv2d(dim, out_channels, kernel_size=3, stride=1, padding=1)
...
class MSFR(nn.Module):
    ...
    def __init__(self, in_c, out_c, stride=1):
        ...
        self.inputs_c0 = nn.Sequential(
            CConv2d(in_c, in_channels, kernel_size=3, stride=1, padding=1), nn.LeakyReLU(inplace=True)
        )

        self.c1 = nn.Sequential(
            CConv2d(self.two, self.two, kernel_size=9, stride=stride, padding=4), nn.LeakyReLU(inplace=True)
        )

        self.c2 = nn.Sequential(
            CConv2d(self.four, self.four, kernel_size=7, stride=stride, padding=3), nn.LeakyReLU(inplace=True)
        )

        self.c3 = nn.Sequential(
            CConv2d(self.eight, self.eight, kernel_size=5, stride=stride, padding=2), nn.LeakyReLU(inplace=True)
        )

        self.c4 = nn.Sequential(
            CConv2d(self.sixteen, self.sixteen, kernel_size=3, stride=stride, padding=1), nn.LeakyReLU(inplace=True)
        )
...
class Noiser(nn.Module):
    def __init__(self, dim, in_channels=1, out_channels=1):
        ...
        self.f0 = nn.Sequential(
            CConv2d(in_channels, dim, kernel_size=3, stride=1, padding=1), nn.LeakyReLU(inplace=True)
        )
